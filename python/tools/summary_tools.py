...
def get_summary(params: Dict[str, Any]) -> str:
    ...
    # Get current bubble if no name specified
    if not idea_name:
        try:
            from tools.bubble_tools import get_current_bubble_db_id
...
def submit_to_req_orchestrator(params: Dict[str, Any]) -> str:
    ...
    current_bubble_id = None
    try:
        from tools.bubble_tools import get_current_bubble_db_id, get_current_bubble
        current_bubble_id = get_current_bubble_db_id()
        if not bubble_name:
...
def sync_shuttle_from_orchestrator(params: Dict[str, Any]) -> str:
    ...
    if not bubble_name:
        try:
            from tools.bubble_tools import get_current_bubble
            current = get_current_bubble()
            if current:
