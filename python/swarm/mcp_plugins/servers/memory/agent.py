    ...
    async with McpWorkbench(server_params) as mcp:
        ...
        try:
            ...
            conv_logger.log_session_start(
                ...
                metadata={
                    "agents": ["MemoryOperator", "QAValidator"],
        ...
        try:
            ...
            async for message in team.run_stream(task=full_prompt):
                ...
                if hasattr(message, 'source') and hasattr(message, 'content'):
                    ...
                    # Pretty print agent dialogue (truncated for console)
                    if source == "MemoryOperator":
                        ...
                        if event_server:
                            event_server.broadcast("agent.message", {
                                ...
                                "role": "operator",
                                "content": content,  # EventServer gets full content
                                "icon": "🔧"
                            })
                    ...
                    # Check for tool calls and capture them for ML logging
            ...
            try:
                if agent_messages:
                    ...
                    operator_msgs = [m for m in agent_messages if m['agent'] == 'MemoryOperator']
                    validator_msgs = [m for m in agent_messages if m['agent'] == 'QAValidator']
            ...
            # Log session end for ML dataset
            try:
                conv_logger.log_session_end(
                    status="completed",
                    summary=final_content[:200] if final_content else "Task completed successfully"
                )
            except Exception:
                pass
