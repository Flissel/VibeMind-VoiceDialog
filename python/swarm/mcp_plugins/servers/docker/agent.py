...
# ========== Defaults ==========
DEFAULT_SYSTEM_PROMPT = """You are an AutoGen Assistant with access to Docker MCP tools.
Use the available Docker tools to complete container management tasks.
...
DEFAULT_DOCKER_OPERATOR_PROMPT = """ROLE: Docker Operator (Docker MCP)
GOAL: Complete Docker container management tasks using available MCP tools.
TOOLS: Use ONLY the available MCP Docker tools (container operations, image management, network, volumes).
GUIDELINES:
...
def get_system_prompt() -> str:
    """Get system prompt for Docker agent."""
...
def get_task_prompt() -> str:
    """Get task prompt for Docker operations."""
...
# ========== Main Entry Point (GitHub/Time Pattern) ==========
async def run_docker_agent(config: DockerAgentConfig):
    """Docker MCP Agent main entry point.

    Follows the SESSION_ANNOUNCE pattern for backend integration.

    Args:
        config: DockerAgentConfig with task, session_id, etc.
    """
    ...
    logger.info("=" * 80)
    logger.info("PORT ANNOUNCEMENT: SESSION_ANNOUNCE (stdout JSON)")
    logger.info(f"  Method: stdout print with JSON")
    logger.info(f"  Session ID: {config.session_id}")
    logger.info(f"  Host: {host}")
    logger.info(f"  Port: {port}")
    logger.info(f"  Timestamp: {announce_timestamp}")
    logger.info("=" * 80)
    ...
    docker_config = None
    for srv in servers:
        if srv.get("name") == "docker" and srv.get("active"):
            docker_config = srv
            break

    if not docker_config:
        event_server.broadcast("error", {"text": "Docker MCP server not found or not active in servers.json"})
    ...
    docker_secrets = secrets.get("docker", {})

    # Prepare environment variables
    env = os.environ.copy()

    # Load secrets from secrets.json
    for key, val in docker_secrets.items():
    ...
    # Create Docker MCP server params
    docker_server_params = StdioServerParams(
    ...
    # Run Society of Mind multi-agent system with Docker workbench
    async with McpWorkbench(docker_server_params) as docker_mcp:
        # Load Society of Mind prompts
        operator_prompt = load_prompt_from_module("docker_operator_prompt", BASE_DIR, DEFAULT_DOCKER_OPERATOR_PROMPT)
        qa_prompt = load_prompt_from_module("qa_validator_prompt", BASE_DIR, DEFAULT_QA_VALIDATOR_PROMPT)

        # Create Docker Operator agent (with Docker MCP workbench)
        # Add model context to maintain conversation history
        docker_operator = AssistantAgent(
            "DockerOperator",
            model_client=task_aware_client,
            workbench=docker_mcp,
        ...
        event_server.broadcast("status", {"text": "Society of Mind: Docker Operator + QA Validator"})
        ...
        try:
            # Extract model name from client
            model_name = "unknown"
            ...
            conv_logger.log_session_start(
                ...
                metadata={
                    "agents": ["Operator", "QAValidator"],
                    "system": "Society of Mind"
                }
            )

            # Log error for ML dataset
            try:
                conv_logger.log_error(
                    error_type=type(e).__name__,
                    error_message=str(e)
                )
                conv_logger.log_session_end(
                    status="error",
...
# Run the agent and stream messages
        print(f"\n{'='*60}")
        print(f"🎭 Society of Mind: Docker Operator + QA Validator")
        print(f"{'='*60}\n")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Docker MCP Agent with Society of Mind")
