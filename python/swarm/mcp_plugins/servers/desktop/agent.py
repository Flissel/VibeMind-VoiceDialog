...
# ========== Defaults ==========
DEFAULT_SYSTEM_PROMPT = """You are an AutoGen Assistant with access to desktop MCP tools.
Use the available tools to complete desktop tasks.
Follow the TOOL USAGE contract strictly and call only the exposed tool names.
"""

DEFAULT_TASK_PROMPT = """Use the available desktop tools to accomplish the goal and stream your progress.
Be clear and concise in your responses.
"""

DEFAULT_OPERATOR_PROMPT = """ROLE: Desktop Operator
GOAL: Complete desktop tasks using available MCP tools.
TOOLS: Use ONLY the available MCP desktop tools.
GUIDELINES:
...
GOAL: Verify that the user's desktop task is completely and correctly fulfilled.
CHECK:
- Were the required desktop operations precisely executed?
...
def get_task_prompt() -> str:
    """Get task prompt for desktop operations."""
...
def init_model_client(task: str = "") -> OpenAIChatCompletionClient:
    ...
    return shared_init_model_client("desktop", task)


# ========== Pydantic Config Model ==========
class DesktopAgentConfig(BaseModel):
    """Configuration for desktop agent execution."""
    task: str
    session_id: str
...
async def run_desktop_agent(config: DesktopAgentConfig):
    """Desktop MCP Agent main entry point.

    Follows the SESSION_ANNOUNCE pattern for backend integration.

    Args:
        config: DesktopAgentConfig with task, session_id, etc.
    """
    # Setup logging with session identifier
    logger = setup_logging(f"desktop_agent_{config.session_id}")

    # Initialize EventServer with session logging
    event_server = EventServer(session_id=config.session_id, tool_name="desktop")
    ...
    tool_config = None
    for srv in servers:
        if srv.get("name") == "desktop" and srv.get("active"):
            tool_config = srv
            break

    if not tool_config:
        event_server.broadcast("error", {"text": "Desktop MCP server not found or not active in servers.json"})
    ...
    tool_secrets = secrets.get("desktop", {})
    ...
    async with McpWorkbench(server_params) as mcp:
        ...
        operator_prompt = load_prompt_from_module("desktop_operator_prompt", BASE_DIR, DEFAULT_OPERATOR_PROMPT)
        ...
        event_server.broadcast("status", {"text": "Society of Mind: Desktop Operator + QA Validator"})
        ...
        print(f"\n{'='*60}")
        print(f"🎭 Society of Mind: Desktop Operator + QA Validator")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            async for message in team.run_stream(task=full_prompt):
                ...
                if hasattr(message, 'source') and hasattr(message, 'content'):
                    ...
                    if source == "DesktopOperator":
                        print(f"\n🔧 DesktopOperator:")
                        print(f"   {content[:500]}{'...' if len(content) > 500 else ''}")
                        if event_server:
                            event_server.broadcast("agent.message", {
                                "agent": "DesktopOperator",
                                "role": "operator",
                                "content": content,
                                "icon": "🔧"
                            })
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Desktop MCP Agent with Society of Mind")
    ...
    # Determine task
    task = args.task or os.getenv("MCP_TASK") or "Complete desktop task"
    ...
    asyncio.run(run_desktop_agent(config))
