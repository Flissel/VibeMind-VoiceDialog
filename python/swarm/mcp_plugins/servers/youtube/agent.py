...
# ========== Defaults ==========
DEFAULT_SYSTEM_PROMPT = """You are an AutoGen Assistant with access to youtube MCP tools.
Use the available tools to complete youtube tasks.
Follow the TOOL USAGE contract strictly and call only the exposed tool names.
"""

DEFAULT_TASK_PROMPT = """Use the available youtube tools to accomplish the goal and stream your progress.
Be clear and concise in your responses.
"""

DEFAULT_OPERATOR_PROMPT = """ROLE: Youtube Operator
GOAL: Complete youtube tasks using available MCP tools.
TOOLS: Use ONLY the available MCP youtube tools.
GUIDELINES:
...
GOAL: Verify that the user's youtube task is completely and correctly fulfilled.
CHECK:
- Were the required youtube operations precisely executed?
...
def get_system_prompt() -> str:
    """Get system prompt for youtube agent."""
...
def get_task_prompt() -> str:
    """Get task prompt for youtube operations."""
...
def init_model_client(task: str = "") -> OpenAIChatCompletionClient:
    ...
    return shared_init_model_client("youtube", task)


# ========== Pydantic Config Model ==========
class YoutubeAgentConfig(BaseModel):
    """Configuration for youtube agent execution."""
    task: str
    session_id: str
...
async def run_youtube_agent(config: YoutubeAgentConfig):
    """Youtube MCP Agent main entry point.

    Follows the SESSION_ANNOUNCE pattern for backend integration.

    Args:
        config: YoutubeAgentConfig with task, session_id, etc.
    """
    # Setup logging with session identifier
    logger = setup_logging(f"youtube_agent_{config.session_id}")

    # Initialize EventServer with session logging
    event_server = EventServer(session_id=config.session_id, tool_name="youtube")
    ...
    tool_config = None
    for srv in servers:
        if srv.get("name") == "youtube" and srv.get("active"):
            tool_config = srv
            break

    if not tool_config:
        event_server.broadcast("error", {"text": "Youtube MCP server not found or not active in servers.json"})
    ...
    tool_secrets = secrets.get("youtube", {})
    ...
    async with McpWorkbench(server_params) as mcp:
        ...
        operator_prompt = load_prompt_from_module("youtube_operator_prompt", BASE_DIR, DEFAULT_OPERATOR_PROMPT)
        ...
        event_server.broadcast("status", {"text": "Society of Mind: Youtube Operator + QA Validator"})
        ...
        print(f"\n{'='*60}")
        print(f"🎭 Society of Mind: Youtube Operator + QA Validator")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            async for message in team.run_stream(task=full_prompt):
                ...
                if hasattr(message, 'source') and hasattr(message, 'content'):
                    ...
                    if source == "YoutubeOperator":
                        print(f"\n🔧 YoutubeOperator:")
                        print(f"   {content[:500]}{'...' if len(content) > 500 else ''}")
                        if event_server:
                            event_server.broadcast("agent.message", {
                                "agent": "YoutubeOperator",
                                "role": "operator",
                                "content": content,
                                "icon": "🔧"
                            })
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Youtube MCP Agent with Society of Mind")
    ...
    # Determine task
    task = args.task or os.getenv("MCP_TASK") or "Complete youtube task"

    # Create config
    config = YoutubeAgentConfig(
        task=task,
    ...
    asyncio.run(run_youtube_agent(config))
