...
# ========== Defaults ==========
DEFAULT_SYSTEM_PROMPT = """You are an AutoGen Assistant with access to windows_core MCP tools.
Use the available tools to complete windows_core tasks.
Follow the TOOL USAGE contract strictly and call only the exposed tool names.
"""

DEFAULT_TASK_PROMPT = """Use the available windows_core tools to accomplish the goal and stream your progress.
Be clear and concise in your responses.
"""

DEFAULT_OPERATOR_PROMPT = """ROLE: Windows Core Operator
GOAL: Complete windows_core tasks using available MCP tools.
TOOLS: Use ONLY the available MCP windows_core tools.
GUIDELINES:
...
GOAL: Verify that the user's windows_core task is completely and correctly fulfilled.
CHECK:
- Were the required windows_core operations precisely executed?
...
def get_system_prompt() -> str:
    """Get system prompt for windows_core agent."""
...
def get_task_prompt() -> str:
    """Get task prompt for windows_core operations."""
...
def init_model_client(task: str = "") -> OpenAIChatCompletionClient:
    ...
    return shared_init_model_client("windows_core", task)


# ========== Pydantic Config Model ==========
class WindowsCoreAgentConfig(BaseModel):
    """Configuration for windows_core agent execution."""
    task: str
    session_id: str
...
async def run_windows_core_agent(config: WindowsCoreAgentConfig):
    """Windows Core MCP Agent main entry point.

    Follows the SESSION_ANNOUNCE pattern for backend integration.

    Args:
        config: WindowsCoreAgentConfig with task, session_id, etc.
    """
    # Setup logging with session identifier
    logger = setup_logging(f"windows_core_agent_{config.session_id}")

    # Initialize EventServer with session logging
    event_server = EventServer(session_id=config.session_id, tool_name="windows_core")

    # Initialize ConversationLogger for ML-ready conversation logs
    conv_logger = ConversationLogger(
        session_id=config.session_id,
        tool_name="windows-core",
    ...
    tool_config = None
    for srv in servers:
        if srv.get("name") == "windows-core" and srv.get("active"):
            tool_config = srv
            break

    if not tool_config:
        event_server.broadcast("error", {"text": "Windows Core MCP server not found or not active in servers.json"})
    ...
    tool_secrets = secrets.get("windows-core", {})
    ...
    async with McpWorkbench(server_params) as mcp:
        ...
        operator_prompt = load_prompt_from_module("windows_core_operator_prompt", BASE_DIR, DEFAULT_OPERATOR_PROMPT)
        ...
        event_server.broadcast("status", {"text": "Society of Mind: Windows Core Operator + QA Validator"})
        ...
        print(f"\n{'='*60}")
        print(f"🎭 Society of Mind: Windows Core Operator + QA Validator")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            async for message in team.run_stream(task=full_prompt):
                ...
                if hasattr(message, 'source') and hasattr(message, 'content'):
                    ...
                    if source == "WindowsCoreOperator":
                        print(f"\n🔧 WindowsCoreOperator:")
                        print(f"   {content[:500]}{'...' if len(content) > 500 else ''}")
                        if event_server:
                            event_server.broadcast("agent.message", {
                                "agent": "WindowsCoreOperator",
                                "role": "operator",
                                "content": content,
                                "icon": "🔧"
                            })
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Windows Core MCP Agent with Society of Mind")
    ...
    # Determine task
    task = args.task or os.getenv("MCP_TASK") or "Complete windows_core task"

    # Create config
    config = WindowsCoreAgentConfig(
        task=task,
    ...
    asyncio.run(run_windows_core_agent(config))
