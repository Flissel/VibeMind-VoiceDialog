...
export function WorkflowBuilder() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGenResult(result?.message ?? 'Workflow erstellt')
      setDescription('')
      refresh() // Reload list
    } catch (e) {
      setGenResult(`Fehler: ${e instanceof Error ? e.message : String(e)}`)
    ...
    <div>
      ...
      <div style={{
        ...
        <textarea
          ...
          placeholder="z.B.: AI Agent mit Webhook-Trigger, PostgreSQL DB und Think Tool der Datev-Daten ausliest..."
          rows={3}
          style={{
        ...
        <div className="flex items-center gap-3" style={{ marginTop: 'var(--space-3)' }}>
          ...
          >
            {generating ? 'Generiere...' : 'Workflow generieren'}
          </button>
          {genResult && (
            <span style={{
              fontSize: 'var(--text-caption1)',
              color: genResult.startsWith('Fehler') ? 'var(--system-red)' : 'var(--system-green)',
            }}>
              {genResult}
            </span>
          )}
      ...
      {loading && workflows.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center" style={{
          ...
          <span style={{ fontSize: 'var(--text-footnote)', color: 'var(--text-tertiary)' }}>
            Beschreibe einen Workflow oben oder sag: "Erstelle einen Workflow fuer..."
          </span>
        </div>
      ) : (
        <div style={{
