...
class MultiverseApp {
    ...
    _showDesktopDashboard() {
        ...
        // --- Face-swap control strip (thin row under eyeTerm strip) ---
        const swapStrip = document.createElement('div');
        swapStrip.id = 'faceswap-strip';

        const swapLabel = document.createElement('div');
        swapLabel.id = 'faceswap-label';
        swapLabel.textContent = 'Face-Swap:';
        swapStrip.appendChild(swapLabel);

        // Dropdown: Random (default) / Off / Alina, Diego, Simone, ... 15 Gesichter
        const swapSelect = document.createElement('select');
        swapSelect.id = 'faceswap-select';
        swapSelect.addEventListener('change', () => this._onFaceswapSelectChange());
        swapStrip.appendChild(swapSelect);

        const swapShuffle = document.createElement('button');
        swapShuffle.id = 'faceswap-shuffle-btn';
        swapShuffle.className = 'faceswap-btn';
        swapShuffle.textContent = '🎲';
        swapShuffle.title = 'Neues Random-Gesicht';
        swapShuffle.addEventListener('click', () => this._pickRandomFace());
        swapStrip.appendChild(swapShuffle);

        const swapStatus = document.createElement('div');
        swapStatus.id = 'faceswap-status';
        swapStrip.appendChild(swapStatus);

        // Record button (right side of strip)
        const recBtn = document.createElement('button');
        recBtn.id = 'faceswap-record-btn';
        recBtn.className = 'faceswap-btn';
        recBtn.textContent = '● Record';
        recBtn.title = 'Record UI frame to ~/.rowboat/Videos/';
        recBtn.addEventListener('click', () => this._toggleRecording());
        swapStrip.appendChild(recBtn);

        panel.appendChild(swapStrip);

        // Face-swap + recording state + render + poll wiring
        this._swapState = { list: [], presets: [], active: null, lastError: null };
        this._recState = { active: false };
        this._swapAutoPicked = false;
        this._renderFaceswapDropdown();
        this._renderRecordingButton();
        this._pollFaceswapStatus();
        if (this._swapPollInterval) clearInterval(this._swapPollInterval);
        this._swapPollInterval = setInterval(() => this._pollFaceswapStatus(), 3000);
    ...
    /**
     * Poll eyeTerm face-swap + recording status.
     * Reads from Automation backend proxy first; falls back to eyeTerm directly.
     */
    async _pollFaceswapStatus() {
        const presetUrls = [
            'http://localhost:8007/api/eyeterm/presets',
            'http://127.0.0.1:8099/presets',
        ];
        let presetsOK = false;
        for (const url of presetUrls) {
            try {
                ...
                if (!r.ok) continue;
                const data = await r.json();
                // presets_v2: [{id, name}, ...]  — fallback to plain ids if missing
                const list = (data.presets_v2 && data.presets_v2.length)
                    ? data.presets_v2
                    : (data.presets || []).map(s => ({ id: s, name: s }));
                const changed =
                    data.active !== this._swapState.active ||
                    JSON.stringify(list) !== JSON.stringify(this._swapState.list || []);
                this._swapState.list = list;
                this._swapState.presets = list.map(p => p.id);   // backward-compat for other code
                this._swapState.active = data.active ?? null;
                this._swapState.lastError = null;
                if (changed) this._renderFaceswapDropdown();
                presetsOK = true;
                break;
            } catch (e) {
                this._swapState.lastError = e.message;
            }
        }
        if (!presetsOK && ((this._swapState.list && this._swapState.list.length) || this._swapState.lastError)) {
            this._swapState.list = [];
            this._swapState.presets = [];
            this._swapState.active = null;
            this._renderFaceswapDropdown();
        }

        // Auto-pick a random preset on first successful poll if user hasn't chosen
        if (presetsOK && !this._swapAutoPicked && this._swapState.list && this._swapState.list.length > 0) {
            this._swapAutoPicked = true;
            if (this._swapState.active === null) {
                this._pickRandomFace();
            }
        }

        // Recording state (separate endpoint, same fallback)
        const recUrls = [
            'http://localhost:8007/api/eyeterm/recording',
            'http://127.0.0.1:8099/recording',
        ];
        for (const url of recUrls) {
            try {
                ...
                const data = await r.json();
                const changed =
                    data.active !== this._recState.active ||
                    data.duration_s !== this._recState.duration_s;
                this._recState = data || { active: false };
                if (changed) this._renderRecordingButton();
                return;
            } catch (e) { /* try next */ }
        }
    }
    ...
    /**
     * Render the face-swap preset dropdown: [Random, Off, face1..faceN].
     * Keeps `_swapState.active` (server-side slug) in sync with the <select> value.
     */
    _renderFaceswapDropdown() {
        const select = document.getElementById('faceswap-select');
        const status = document.getElementById('faceswap-status');
        if (!select) return;

        const offline = !this._swapState.list || this._swapState.list.length === 0;
        if (offline) {
            select.innerHTML = '<option value="__offline__">offline</option>';
        ...
        // Build options
        const opts = [];
        opts.push({ value: '__random__', label: '🎲 Random' });
        opts.push({ value: '__off__', label: 'Off (no swap)' });
        for (const p of this._swapState.list) {
            opts.push({ value: p.id, label: p.name });
        }

        // Only rebuild if option set changed (preserves selection during polls)
        ...
        // Sync value with active state (if user hasn't manually selected Random)
        const selectedVal = select.value;
        if (selectedVal !== '__random__') {
            const desired = this._swapState.active === null ? '__off__' : this._swapState.active;
        ...
        if (status) {
            if (selectedVal === '__random__' && this._swapState.active) {
                const activeEntry = this._swapState.list.find(p => p.id === this._swapState.active);
                status.textContent = `🎲 ${activeEntry ? activeEntry.name : this._swapState.active}`;
            } else if (this._swapState.active) {
                const activeEntry = this._swapState.list.find(p => p.id === this._swapState.active);
                status.textContent = activeEntry ? activeEntry.name : this._swapState.active;
            } else {
    ...
    /**
     * User changed the dropdown — route to off / random / specific preset.
     */
    async _onFaceswapSelectChange() {
        const select = document.getElementById('faceswap-select');
        if (!select || select.disabled) return;
        const val = select.value;
        if (val === '__offline__') return;
        if (val === '__random__') {
            this._pickRandomFace();
            return;
        }
        const targetName = (val === '__off__') ? null : val;
        try {
            await this._sendFaceswapCommand(targetName);
            this._swapState.active = targetName;
            this._renderFaceswapDropdown();
        } catch (e) {
            const status = document.getElementById('faceswap-status');
    ...
    /**
     * Pick a random face different from the current one. Called by dice button
     * and auto-selected on first poll.
     */
    async _pickRandomFace() {
        const list = (this._swapState.list || []).filter(p => p.id !== this._swapState.active);
        if (list.length === 0) return;
        const pick = list[Math.floor(Math.random() * list.length)];
        try {
            await this._sendFaceswapCommand(pick.id);
            this._swapState.active = pick.id;
            // Keep dropdown on "Random" so shuffle keeps working
            const select = document.getElementById('faceswap-select');
            if (select && !select.disabled) select.value = '__random__';
            this._renderFaceswapDropdown();
        } catch (e) {
            const status = document.getElementById('faceswap-status');
            if (status) status.textContent = 'random pick failed';
        }
    }

    /**
     * Send preset command via Automation backend (falls back to eyeTerm direct).
     */
    async _sendFaceswapCommand(name) {
        const payloads = [
            { url: 'http://localhost:8007/api/eyeterm/set-preset', body: { name } },
            { url: 'http://127.0.0.1:8099/command', body: { type: 'set_preset', name } },
        ];
        let lastErr = null;
    ...
    /**
     * Render the Record button state (idle vs recording).
     */
    _renderRecordingButton() {
        const btn = document.getElementById('faceswap-record-btn');
        if (!btn) return;
        if (this._recState.active) {
            btn.classList.add('recording');
            const dur = typeof this._recState.duration_s === 'number'
                ? ` ${Math.floor(this._recState.duration_s)}s`
                : '';
            btn.textContent = `■ Stop${dur}`;
            btn.title = this._recState.filename
                ? `Recording to ${this._recState.filename}`
                : 'Stop recording';
        } else {
            btn.classList.remove('recording');
            btn.textContent = '● Record';
            btn.title = 'Record UI frame to ~/.rowboat/Videos/';
        }
    }
    ...
    async _toggleRecording() {
        ...
        const starting = !this._recState.active;
        const hint = starting && this._swapState.active ? this._swapState.active : null;
        const endpoints = starting
            ? [
                { url: 'http://localhost:8007/api/eyeterm/recording/start', body: { name_hint: hint } },
                { url: 'http://127.0.0.1:8099/command', body: { type: 'start_recording', name_hint: hint } },
            ]
            : [
                { url: 'http://localhost:8007/api/eyeterm/recording/stop', body: {} },
                { url: 'http://127.0.0.1:8099/command', body: { type: 'stop_recording' } },
            ];
        for (const ep of endpoints) {
            try {
                ...
                if (r.ok) {
                    // Optimistic flip, poll will confirm
                    this._recState.active = starting;
                    this._renderRecordingButton();
                    break;
                }
            } catch (e) { /* try next */ }
    ...
    exitDesktopDashboard() {
        ...
        // Stop face-swap polling
        if (this._swapPollInterval) {
            clearInterval(this._swapPollInterval);
            this._swapPollInterval = null;
        }
